...
def main(
    ...
    minsize: int = Option(
        1, "--min-size", "-m", help="Ignore files smaller than MIN_SIZE"
    ),
    symlinks: bool = Option(
        False,
        "--enable-symlinks",
        "-l",
        help="Include symlinks in duplication report",
    ),
    glob=Option("*", "--glob", "-g", help="Limit matches to glob pattern"),
    verbose=Option(
        False, "--verbose", "-v", help="Display progress information on STDERR"
    ),
) -> None:
    opts = Options(maxsize, minsize, symlinks, glob, verbose)
    find_duplicates(dirs, opts)
...
def get_path_infos(
    dirs: Iterable[str | PathLike[Any]], opts: Options
) -> Iterator[Finfo]:
